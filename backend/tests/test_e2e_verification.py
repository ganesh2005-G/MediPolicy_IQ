...
def test_full_e2e_flow():
    with TestClient(app) as client:
        ...
        res = client.post("/api/v1/ocr/process", data={"doc_type": "INVOICE", "sample_type": "inpatient_bill"})
        assert res.status_code == 200
        ocr_data = res.json()
        assert "document_code" in ocr_data

        # 7. RAG AI Query
        res = client.post("/api/v1/rag/query", json={"query": "What is the room rent cap?"})
        assert res.status_code == 200
        rag_data = res.json()
